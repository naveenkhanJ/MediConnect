...
// Deduplicate specialties for the dropdown
...
export default function DoctorsBySpecialtyPage() {
  ...
  const bySpecialty = useMemo(
    () => doctors.filter((d) => d.speciality.toLowerCase() === decodedSpecialty.toLowerCase()),
    [decodedSpecialty]
  );

  const visibleDoctors = useMemo(
    () =>
      bySpecialty.filter((d) =>
      ...
      <div className="max-w-4xl mx-auto mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5">{visibleDoctors.length} doctor(s) shown</p>
          </div>

          {/* Specialty Switcher Dropdown */}
      ...
      <div className="max-w-4xl mx-auto flex flex-col gap-4">
        {visibleDoctors.length === 0 ? (
          <div className="bg-white rounded-xl p-10 text-center text-gray-400">
            {search
              ? `No doctors named "${search}" in ${decodedSpecialty}.`
